...
![mysql-architecture](Temp-Cloud笔记图片/mysql-architecture.png)
...
MySQL的存储引擎是表类型，决定了表的数据结构和索引结构以及它们在磁盘上的组织方式，负责数据的存储、索引、检索、修改操作的实现，并对事务、锁、外键等特性提供支持

自MySQL 5.5.5起，InnoDB成为默认存储引擎
...
- InnoDB
  ...
  支持事务、行锁、外键约束、聚簇索引、缓冲池、崩溃恢复等完善的功能，性能最好

- MyISAM

  传统引擎，仅支持表锁，不支持事务，支持压缩表

- Memory（HEAP）

  内存存储表数据，提供极高的访问性能，仅支持表锁，不支持事务，不提供数据持久化

- Archive

  提供极高压缩率，但仅支持插入、查询和非物理删除，不支持数据更新，查询性能极差
...
- Federated
  ...
  不存储数据，提供连接到远程MySQL服务器物理表的虚拟表，跨网络开销影响性能

- NDB（MySQL Cluster）

  用于构建高可用、高冗余的分布式内存数据库

##### InnoDB

InnoDB架构图

![innodb-architecture-8-0](Temp-Cloud笔记图片/innodb-architecture-8-0.png)
...
MySQL中的锁用于并发访问时保障数据的一致性和完整性，包含以下类型：

- 全局锁
- 表级锁
- 行级锁

在MySQL中可以通过表`performance_schema.date_locks`查看所有已持有或正在请求的锁



#### 全局锁

全局锁用于锁定整个数据库实例（`database`/`schema`），使该数据库只读，所有非读操作将被阻塞
...
全局锁常用于备份或迁移（`mysqldump`），但由于会造成严重的性能影响，且在主从架构中可能造成更多的问题，常常被其他方式替代（如`mysqldump --single-transaction`、`MySQL Enterprise Backup`、`Percona XtraBackup`等）
...
表锁用于对整个表加锁，以用于表级别的数据一致性和完整性保障

表锁是会话级别的

<u>表共享锁</u>

又称表读锁，允许多个持有表共享锁的会话同时进行读操作（`SELECT`），但不允许任何会话进行写操作（`INSERT`、`UPDATE`、`DELETE`）

使用如下语句对表加共享锁

持有表共享锁的会话在锁期间进行写入操作将发生错误，而其他会话在试图获取表独占锁时将阻塞

<u>表独占锁</u>

又称表排它锁、表写锁，仅允许一个会话持有表独占锁并对表进行读或写操作（`SELECT`、`INSERT`、`UPDATE`、`DELETE`等），不允许其他任何会话进行任何读或写操作

使用如下语句对表加独占锁

当一个会话持有一个表的独占锁，其他任何会话试图获取表共享锁或独占锁时都将阻塞

<u>表锁的获取与释放</u>

一个会话可以通过以下语句显式地获取一个或多个表的共享锁或独占锁
...
> [!NOTE]
>
> - `LOCK TABLES`语句或释放已经获取的所有表锁
> - 在InnoDB引擎中，`LOCK TABLES`语句会强制提交当前事务
> - 在MyISAM引擎中不支持行级锁，所有读写操作自动加表锁
> - 当会话终止时，所有表锁都将释放

##### 元数据锁

元数据锁在MySQL 5.5开始引入，用于避免DDL和DML操作的冲突，避免对表结构的修改影响表数据读写操作的一致性，保证结构定义（表、试图、存储过程等）等元数据和表数据的一致性和完整性

元数据锁是事务级别的

元数据锁在事务执行期间自动获取，并在事务提交或回滚时自动释放

元数据锁的粒度较多，但总体分共享锁和排它锁

- 共享锁

  `SELECT`、`INSERT`、`UPDATE`、`DELETE`、`LOCK TABLES`、`SHOW CREATE TABLE`等不修改结构定义的语句将自动地获取共享锁

  共享锁支持多个持有共享锁的事务同时读取结构定义等元数据

- 排它锁

  `ALTER TABLE`、`DROP TABLE`、`TRUNCATE TABLE`、`CREATE INDEX`、`DROP INDEX`及部分影响表结构的`CREATE/DROP TRIGGER/PROCEDURE/FUNCTION`的语句将自动地获取排他锁

  排它锁仅允许一个事务持有，并对结构定义等元数据进行读或写操作

##### 意向锁

意向锁用于协调不同粒度锁（行锁与表锁）之间的关系，避免获取表锁时对表的逐行扫描

> 考虑以下情景：
>
> 事务A对表user（InnoDB存储引擎）执行了语句`UPDATE user SET name = 'Alice' WHERE id = 65535`并准备提交，而事务B此时试图获取表user的写锁。由于事务A实质上持有表user行`id = 65536`的写锁，事务B在获取表user的写锁前需要扫描该表的所有行，以确保事务B能够获取该表的锁
>
> 如果表user上具有某种类型的“标志”，使其存储当前表中是否已经有事务获取了行锁，那么其他事务在试图获取表锁时，仅需读取该“标志”，即可判断是否可以获取表锁

意向锁是事务级别的

意向锁属于表级锁的一种，但其服务于行锁。当一个事务试图获取某行的行锁之前，会先获取该行所在表的意向锁
...
| 请求类型 | 现有锁状态 (`IS`) | 现有锁状态 (`IX`) | 现有锁状态 (`S`) | 现有锁状态 (`X`) |
| -------- | ----------------- | ----------------- | ---------------- | ---------------- |
| 申请`IS` | 兼容              | 兼容              | 兼容             | 冲突             |
| 申请`IX` | 兼容              | 兼容              | 冲突             | 冲突             |
| 申请`S`  | 兼容              | 冲突              | 兼容             | 冲突             |
